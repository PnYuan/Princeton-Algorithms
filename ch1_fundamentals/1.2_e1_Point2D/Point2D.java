...
public final class Point2D implements Comparable<Point2D>{
    ...
    /**
     * Initializes a new point (x, y). Constructor
    ...
    public static void main(String[] args) {
        ...
        StdDraw.setCanvasSize(500, 500);  //setting the draw's properties
        StdDraw.setXscale(0, 100);
        StdDraw.setYscale(0, 100);
        StdDraw.setPenRadius(0.005);
        StdDraw.enableDoubleBuffering();
        
        Point2D[] points = new Point2D[n];  //new a Point2D object
        
        for (int i = 0; i < n; i++) {  //generateing N 2D points AND draw
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if(i == j) continue;
                dis2 = points[i].distanceSquaredTo(points[j]);
                if(dis2 < dis){
                    dis = dis2;
                    i_temp = i;
                    j_temp = j;
        ...
        StdDraw.setPenColor(StdDraw.RED);  //linking closest pair of points.
        points[i_temp].drawTo(points[j_temp]);
        StdDraw.show();  
    }
